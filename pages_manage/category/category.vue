...
<script>
	...
	export default {
		...
		methods:{
			//获取数据库中的分类
			getCateGory(){
				db.collection("mall-category").get().then(res=>{
					console.log(res);
			...
			async dialogConfirm(e){
				this.iptValue = e;
				if(this.updateID){
					await db.collection("mall-category").doc(this.updateID).update({
						name:this.iptValue
					})		
				}else{
					await db.collection("mall-category").add({
						name:this.iptValue
					})
				}							
				this.getCateGory();					
			...
			deleteData(id){
				uni.showModal({
					...
					success:res=>{
						if(res.confirm){
							db.collection("mall-category").doc(id).remove().then(res=>{
								this.getCateGory();
							})							
						}
					},
