<template>
	...
	<view class="orderPage">		
		...
		<view class="row" v-for="item in orderList" :key="item._id" @click="goDetail(item)">
			...
			<view class="btnGroup">
				<button type="primary" :disabled="item.done" size="mini" @click.stop="orderDone(item._id)">完成</button>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			//获取订单列表
			async getOrderList(){
				let res = await orderCloudObj.getAllList();
				console.log(res);
			...
			//点击完成订单
			orderDone(id){
				orderCloudObj.updateDone(id).then(res=>{
					this.getOrderList();
				})
			}
		}
